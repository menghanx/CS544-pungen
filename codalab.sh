...
#python scripts/parsed_to_tokenized.py --input data/valid.txt --output /tmp/valid.tokenized.txt;
#cat /tmp/train.tokenized.txt /tmp/valid.tokenized.txt > /tmp/sent.tokenized.txt;
#python generate_pun.py combiner-data --path models/checkpoint_best.pt --beam 20 --nbest 1 --unkpen 100 --task edit --system rule+neural --retriever-model retriever.pkl --doc-file /tmp/sent.tokenized.txt --skipgram-model skipgram/dict.txt skipgram/model.pt --num-candidates 500 --num-templates 100 --num-topic-word 100 --type-consistency-threshold 0.3 --pun-words pun-data/test.json --outdir results/Retrieve+Swap+Topic+Smoother --scorer random --max-num-examples 100 --word-counts-path word-counts/dict.txt

python scripts/parsed_to_tokenized.py --input data/train.txt --output /tmp/train.tokenized.txt;
python scripts/parsed_to_tokenized.py --input data/valid.txt --output /tmp/valid.tokenized.txt;
cat /tmp/train.tokenized.txt /tmp/valid.tokenized.txt > /tmp/sent.tokenized.txt;
python generate_pun.py combiner-data --path models/checkpoint_best.pt --beam 20 --nbest 1 --unkpen 100 --task edit --source-lang src --target-lang tgt --system rule --retriever-model retriever.pkl --doc-file /tmp/sent.tokenized.txt --skipgram-model skipgram/dict.txt skipgram/model.pt --num-candidates 500 --num-templates 100 --num-topic-word 100 --type-consistency-threshold 0.3 --pun-words pun-data/test.json --outdir results/new2_data2 --scorer random --max-num-examples 250 --word-counts-path word-counts/dict.txt --lm-path models/wikitext/wiki103.pt